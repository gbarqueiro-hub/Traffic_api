...
class CarPassagesLast24hViewTest(APITestCase):
    def setUp(self):
        self.admin = User.objects.create_superuser('admin1', 'admin@test.com', '1234')
        ...
        self.segment = RoadSegment.objects.create(
            long_start=0, lat_start=0, long_end=1, lat_end=1
        )

        Passage.objects.create(
            car=self.car, sensor=self.sensor, road_segment=self.segment, timestamp=timezone.now()
        )
